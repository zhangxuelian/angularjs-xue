describe('pagination directive', function () {
    ...
    beforeEach(module('xue.pagination'));
    beforeEach(module('xue/template/pagination/pagination.html'));
    beforeEach(inject(function (_$compile_, _$rootScope_) {
        $compile = _$compile_;
        $rootScope = _$rootScope_;
        $rootScope.total = 47; // 5 pages
        $rootScope.currentPage = 3;
        element = $compile('<xue-pagination total-items="total" ng-model="currentPage"></xue-pagination>')($rootScope);
        $rootScope.$digest();
    }));
    ...
    xit('changes currentPage if a page link is clicked', function () {
        clickPaginationEl(2);
        expect($rootScope.currentPage).toBe(2);
    });

    xit('changes currentPage if the "previous" link is clicked', function () {
        clickPaginationEl(0);
        expect($rootScope.currentPage).toBe(2);
    });

    xit('changes currentPage if the "next" link is clicked', function () {
        clickPaginationEl(-1);
        expect($rootScope.currentPage).toBe(4);
    });

    xit('does not change the current page on "previous" click if already at first page', function () {
        updateCurrentPage(1);
        clickPaginationEl(0);
        expect($rootScope.currentPage).toBe(1);
    });

    xit('does not change the current page on "next" click if already at last page', function () {
        updateCurrentPage(5);
        clickPaginationEl(-1);
        expect($rootScope.currentPage).toBe(5);
    });

    xit('changes the number of pages when `total-items` changes', function () {
    ...
    xit('does not "break" when `total-items` is undefined', function () {
    ...
    xit('does not "break" when `total-items` is negative', function () {
        $rootScope.total = -1;
        $rootScope.$digest();
    ...
    xit('does not change the current page when `total-items` changes but is valid', function () {
    ...
    xdescribe('`items-per-page`', function () {
        beforeEach(function () {
            $rootScope.perpage = 5;
    ...
    xdescribe('executes  `ng-change` expression', function () {
        beforeEach(function () {
    ...
    xdescribe('when `page` is not a number', function () {
    ...
    xdescribe('with `max-size` option', function () {
        beforeEach(function () {
    ...
    xdescribe('with `max-size` option & no `rotate`', function () {
        beforeEach(function () {
    ...
    xdescribe('pagination directive with `boundary-links`', function () {
        beforeEach(function () {
    ...
    xdescribe('pagination directive with just number links', function () {
        beforeEach(function () {
    ...
    xdescribe('with just boundary & number links', function () {
        beforeEach(function () {
    ...
    xdescribe('setting `paginationConfig`', function () {
    ...
    xdescribe('override configuration from attributes', function () {
        beforeEach(function () {
